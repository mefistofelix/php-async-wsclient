...
bool ws_commit_upgrade(websocket_object *w, bool install_session)
{
     ...
     * the client offered it. Shared decision for H1 and H2. pmce_bits is
     * the negotiated server deflate window (0 = off, 9..15 = on). */
    int pmce_bits = 0;
#ifdef HAVE_HTTP_COMPRESSION
    {
        ...
        if (cfg != NULL && cfg->ws_permessage_deflate &&
            ...
            websocket_upgrade_object *u =
                websocket_upgrade_from_obj(Z_OBJ(w->upgrade_zv));
            if (u->req != NULL) {
                pmce_bits = ws_pmce_negotiate(u->req);
            }
        }
    }
#endif
    ...
    if (w->h2_stream != NULL) {
        if (!http2_ws_accept(w->h2_stream, w, subprotocol, pmce_bits)) {
            ws_throw_closed(1006, NULL,
    ...
    /* Degenerate path (handler exited without touching the WS API):
     * the 101 is on the wire, but we deliberately install no session,
     * keepalive timer, or read subscription — the dispose that called
     * us is about to tear the connection down. Avoids arming reactor
     * handles only to immediately free them (and the leak that causes). */
    if (!install_session) {
        w->committed = true;
        return true;
    }
    ...
    if (pmce_bits > 0 && !ws_session_enable_pmce(session, pmce_bits)) {
        ws_throw_closed(1006, NULL,
     ...
     * H1 keep-alive (don't dispatch a 2nd request mid-handler), wrong for
     * WS where every inbound frame must keep feeding ws_feed (which never
     * dispatches a new handler). Clear it so the read callback feeds the
     * WS strategy. No read re-arm is needed: conn->io is MULTISHOT (set at
     * accept) and stays armed across the upgrade; re-arming would mint a
     * second io_req and hijack read_cb->active_req, filtering out the real
     * multishot completion (the old branch re-armed only because the
     * pre-refactor read loop stopped the multishot on request-complete). */
...
ZEND_METHOD(TrueAsync_WebSocket, ping)
{
    ...
    /* RFC 6455 §5.5: control-frame payload is capped at 125 bytes. */
    const uint8_t *data = NULL;
    size_t len = 0;
    if (payload != NULL && ZSTR_LEN(payload) > 0) {
        if (ZSTR_LEN(payload) > 125) {
            zend_argument_value_error(1,
                "WebSocket PING payload must not exceed 125 bytes");
            RETURN_THROWS();
        }
        data = (const uint8_t *)ZSTR_VAL(payload);
        len  = ZSTR_LEN(payload);
    }

    ws_session_t *const s = w->session;
    const struct wslay_event_msg ping = {
        .opcode     = WSLAY_PING,
        .msg        = data,
        .msg_length = len,
    };
    if (wslay_event_queue_msg(s->ctx, &ping) != 0) {
        zend_throw_exception_ex(websocket_exception_ce, 0,
            "WebSocket ping queue failed (out of memory or closed)");
        return;
    }
 ...
 * be minted, captured or carried into a handler; see topic_hub.h.
 *
...
static topic_hub_t *ws_topic_hub_of(const websocket_object *w)
{
    if (w->session != NULL) {
    ...
    return conn != NULL ? http_server_get_topic_hub(conn->server) : NULL;
}
...
ZEND_METHOD(TrueAsync_WebSocket, subscriberCount)
{
    ...
    const uint32_t total = topic_hub_count(ws_topic_hub_of(w),
...
void ws_php_classes_register(void)
{
    ...
    websocket_handlers.offset    = offsetof(websocket_object, std);
    ...
    websocket_message_handlers.offset    = offsetof(websocket_message_object, std);
    ...
    websocket_upgrade_handlers.offset    = offsetof(websocket_upgrade_object, std);
